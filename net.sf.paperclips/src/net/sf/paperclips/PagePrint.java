 ...
 * capabilities.
 * <p>
 * PagePrint is horizontally and vertically greedy. Greedy prints take up all the available space on the
 * page.
 * <p>
 * <b>Note:</b> Avoid wrapping PagePrint in prints with space-optimizing semantics (e.g. ColumnPrint
 * equalizes columns on the last page), as this may cause the total page count to be incorrect on some pages.
 * At this time there is no known fix. If wrapping a PagePrint is unavoidable, consider using a custom
 * PageNumberFormat which does not display the total page count.
 * 
 * @author Matthew Hall
 */
public class PagePrint implements Print {
   ...
   * @param headerGap the gap between the header and body, in points.
   ...
   * @param points the new gap between the header and body, expressed in points. 72 points = 1".
   */
...
class PageIterator implements PrintIterator {
  ...
  PageIterator( PageIterator that ) {
    ...
    // FIXME: Wrapping PagePrint in a class with space-optimizing semantics (ColumnPrint) can fork the total
    // page count. i.e. if the copied PageIterator is chosen as the optimal layout, then previous pages will
    // have a page number spawned from a different page numberer. Thus the total page count for those
  ...
  public PrintPiece next( int width, final int height ) {
    ...
    if ( header != null ) {
      ...
      if ( headerPrint != null ) {
        headerPiece = getDecorationPrintPiece( headerPrint, width, availableHeight );
        if ( headerPiece == null )
          return null;
    ...
    PrintPiece result = createResult( height, headerPiece, bodyPiece, footerPiece );
    advancePageNumber();
    return result;
  }
  ...
  private void advancePageNumber() {
    // Null the pageNumber field so the next iteration advances to the next page.
    pageNumber = null;
  }
  ...
  private PrintPiece createResult( int height,
