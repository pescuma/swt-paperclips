...
public abstract class BasicGridLookPainter implements GridLookPainter {
  ...
  public void paint(GC gc,
  	...
  	for (int rowIndex = 0; rowIndex < bodyRows.length; rowIndex++) {
  		...
  		for (int cellIndex = 0; cellIndex < bodyCellSpans[rowIndex].length; cellIndex++) {
  			...
  			paintBodyCell(gc, new Rectangle(X,Y,W,H), firstRowIndex + rowIndex, col, cellSpan, rowTopOpen, rowBottomOpen);
   ...
   * @param col the column offset of the cell within the header.
   * @param colspan the number of columns that this cell spans.
   * @param topOpen whether the cell should be drawn with the top edge of the cell border "open."
   *        An open top border is a visual cue that the cell is being continued from the previous
   *        page.
   * @param bottomOpen whether the cell should be drawn with the bottom edge of the cell border
   * 			  "open."  An open bottom border is a visual cue that the cell will be continued on the
   *        next page. 
   */
  protected abstract void paintBodyCell(GC gc, Rectangle bounds, int row, int col, int colspan,
