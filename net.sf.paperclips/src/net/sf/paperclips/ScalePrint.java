 ...
 * candidate 3 (2006-04-28). Prior to this release, using ScalePrint triggers the bug, causing the document
 * to scale very large on paper. This bug manifests itself only on paper, not with on-screen viewing.
 * @author Matthew Hall
 */
public class ScalePrint implements Print {
  ...
  /**
   * Constructs a ScalePrint which scales down it's target to print at it's preferred size. This constructor
   * is equivalent to calling new ScalePrint(target, null).
...
class ScaleIterator implements PrintIterator {
  ...
  ScaleIterator( ScalePrint print, Device device, GC gc ) {
    ...
    if ( scale == null ) { // auto-scale
      ...
      preferredSize = new Point( (int) Math.ceil( pref.x * s ), (int) Math.ceil( pref.y * s ) );
    }
  }
