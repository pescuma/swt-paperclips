  ...
  private void adjustCellsForColumnInsert( List rows, int index, int count ) {
    for ( int rowI = 0; rowI < rows.size(); rowI++ ) {
      ...
      for ( int cellI = 0; cellI < row.size(); cellI++ ) {
        ...
        // Adjust the cell which extends through the insert point, or whose right side touches the insert
        // point. Except on the last row, don't adjust the final cell if it only touches the insert point
        // (the user may be adding columns right before s/he adds column headers).
