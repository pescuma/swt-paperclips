 ...
 * child prints are laid out into those columns by invoking the add(...) methods.
 * <p>
 * GridPrint uses a column sizing algorithm based on the <a
 * href=http://www.w3.org/TR/html4/appendix/notes.html#h-B.5.2.2>W3C recommendation</a> for automatic layout
 ...
 * <ol>
 * <li>Default size. The column will be somewhere between it's minimum and preferred width. GridPrint will
 * determine the optimum widths for all default size columns, using the modified W3C recommendation described
 * above. This is the recommended option for most cases.
 * <li>Preferred size. The column will be sized to it's preferred width. This option is sometimes
 * appropriate, for example when certain portions of text should not be allowed to line-wrap. In cases where
 * only a few cells in a column need to be prevented from line wrapping, consider wrapping them in a
 * NoBreakPrint instead.
 * <li>Explicit size. The column will be the size you specify, expressed in points. 72 points = 1".
 * </ol>
 * Example: GridPrint grid = new GridPrint("d, p, 72pts");
 * <p>
 * In addition, any column can be given a grow attribute. In the event a grid is not as wide as the page,
 * those columns with the grow attribute set will be widened to fill the extra space.
 * <p>
 * Because GridPrint scales columns according to their minimum sizes in the worst-case scenario, the absolute
 * minimum size of a GridPrint is dependant on its child prints and is not clearly defined.
 * <p>
 * If a grid has one of more columns with the grow attribute set, the grid is horizontally greedy. Greedy
...
public final class GridPrint implements Print {
  ...
  /**
   * Constant column size value indicating that the column should be given its preferred size. (In the
   * context of W3C's autolayout recommendation, this has the effect of setting the columns minimum width to
   * its preferred width. This value is used in the GridColumn constructor.
   */
  ...
  /**
   * Two-dimension list of all footer cells. Each element of this list represents a row in the footer. Each
   * element of a row represents a cellspan in that row.
   */
  // List <List <GridCell>>
   ...
   * Constructs a GridPrint with the given columns and spacing.
   * @param columns a comma-separated list of column specs.
   * @param horizontalSpacing the horizontal spacing (in points) between grid cells.
   ...
   * colspan expanded to fill the added column.
  ...
  /**
   * Inserts the column at the specified position in the grid. Any cells which have been added to the grid
   * prior to adding the column will be adjusted as follows: on each row, the cell which overlaps or whose
   * right edge touches the insert position will be expanded to fill the added column.
  ...
  /**
   * Adds the columns on the right edge of the grid. Any cells which have been added to the grid prior to
   * adding the columns will be adjusted as follows: the right-hand cell of each completed row will have it's
   * colspan expanded to fill the added columns.
  ...
  /**
   * Inserts the columns at the specified position in the grid. Any cells which have been added to the grid
   * prior to adding the columns will be adjusted as follows: on each row, the cell which overlaps or whose
   * right edge touches the insert position will be expanded to fill the added columns.
   * @param index the insert position.
  ...
  /**
   * Separates the comma-separated argument and parses each piece to obtain an array of GridColumns.
  ...
  /**
   * Returns an array containing the header cells in this grid. Each inner array represents one row in the
   * header.
  ...
  /**
   * Returns an array containing the footer cells in the grid. Each inner array represents one row in the
   * footer.
  ...
  /**
   * Sets the column groups to the given two-dimension array. Each int[] array is a group. Columns in a group
   * will be the same size when laid out on the print device.
   * <p>
   * The following statement causes columns 0 and 2 to be the same size, and columns 1 and 3 to be the same
   * size.
   * 
   * <pre>
   ...
   * <p>
   * <b>Note:</b> Column grouping is enforced <i>before</i> column weights. Therefore, columns in the same
  ...
  /**
   * Returns the grid's look. A GridLook determines what decorations will appear around the grid's contents.
   * Default is a DefaultGridLook with no cell spacing, no cell borders, and no background colors.
...
class GridIterator implements PrintIterator {
  ...
  int computeColumnSize( GridCellIterator entry, GridColumn col, PrintSizeStrategy strategy ) {
    ...
    for ( int rowIndex = 0; rowIndex < rows.length; rowIndex++ ) {
      ...
      for ( int cellIndex = 0; cellIndex < row.length; cellIndex++ ) {
        ...
        if ( entry.colspan == 1 && !isExplicitSize( columns[col] ) ) {
          colSizes[col] = Math.max( colSizes[col], computeColumnSize( entry, columns[col], strategy ) );
        }
        col += entry.colspan;
      }
    }
  ...
  private Point computeSize( PrintSizeStrategy strategy, int[] colSizes ) {
    ...
    int width =
        margins.getLeft() + ( colSizes.length - 1 ) * margins.getHorizontalSpacing() + margins.getRight();
    ...
    if ( header.length > 0 ) {
      height =
          margins.getHeaderTop() + margins.getHeaderVerticalSpacing() * ( header.length - 1 )
  ...
  private int[] computeAdjustedColumnSizes( int width ) {
    ...
    width =
        width - margins.getLeft() - margins.getHorizontalSpacing() * ( columns.length - 1 )
            - margins.getRight();
  ...
  /**
   * Iterates through the current row and returns a CompositeEntry array of the PrintPieces generated. This
   * method modifies the contents of the row argument, so backup copies should be made of each cell iterator
   * before passing a row into this method.
   ...
   * @param bottomOpen whether the cell border is open at the bottom. If false, this method must return null
   *        if one or more cells cannot consume all its content in this iteration.
   * @param rowHeight an int array 1 element long for reporting the height of the row back to the caller.
   * @param hasNext a boolean array 1 element long for reporting whether the cells in the row have more
   *        content to display.
   * @return A CompositeEntry array resulting from the iteration. Returns null if the iteration failed. This
   *         happens if bottomOpen is false and one or more cells could not be consume within the available
   *         area.
   */
  CompositeEntry[] nextRow( final GridCellIterator[] row,
    ...
    for ( int cellIndex = 0; cellIndex < row.length; cellIndex++ ) {
      ...
      if ( hAlignment == SWT.DEFAULT )
        hAlignment = columns[col].align;
    ...
    // Now that we have the final row height, loop through again to determine the y offsets of each cell, and
    // to layout
    ...
    for ( int cellIndex = 0; cellIndex < rowPieces.length; cellIndex++ )
      if ( rowPieces[cellIndex] != null )
        result.add( new CompositeEntry( rowPieces[cellIndex], new Point( xOffsets[cellIndex],
                                                                         yOffsets[cellIndex] ) ) );
  ...
  public PrintPiece next( int width, int height ) {
    ...
    if ( headerPresent ) {
      ...
      for ( int rowIndex = 0; rowIndex < header.length; rowIndex++ ) {
        ...
        CompositeEntry[] rowEntries = nextRow( row, colSizes, height, y, false, rowHeight, hasNext );
    ...
    if ( footerPresent ) {
      ...
      for ( int rowIndex = 0; rowIndex < footer.length; rowIndex++ ) {
        ...
        CompositeEntry[] rowEntries = nextRow( row, colSizes, height, footerY, false, rowHeight, hasNext );
    ...
    while ( hasNext() ) {
      ...
      CompositeEntry[] rowEntries =
          nextRow( thisRow, colSizes, height - bodyBottomSpacingClosed, y, bottomOpen, rowHeight, hasNext );
      ...
      if ( ( rowEntries == null || hasNext[0] ) && cellClippingEnabled ) {
        ...
        bottomOpen = true;
        rowEntries = nextRow( thisRow, colSizes, height - bodyBottomSpacingOpen, y, true, rowHeight, hasNext );
      }
    ...
    PrintPiece lookPiece =
        new GridLookPainterPiece( look,
                                  colSizes,
                                  headerHeights,
                                  headerColSpans,
                                  firstRowIndex,
    ...
    Point size =
        new Point( margins.getLeft() + margins.getHorizontalSpacing() * ( colSizes.length - 1 )
            + margins.getRight(), 0 );
...
class GridLookPainterPiece implements PrintPiece {
  ...
  GridLookPainterPiece( GridLookPainter look,
    ...
    this.size =
        new Point( margins.getLeft() + margins.getHorizontalSpacing() * ( colSizes.length - 1 )
            + margins.getRight(), ( headerPresent ? margins.getHeaderTop()
            + margins.getHeaderVerticalSpacing() * ( headerRows.length - 1 ) : 0 )
            + margins.getBodyTop( headerPresent, topOpen )
            + margins.getBodyVerticalSpacing()
            * ( bodyRows.length - 1 )
            + margins.getBodyBottom( footerPresent, bottomOpen )
            + ( footerPresent ? margins.getFooterVerticalSpacing() * ( footerRows.length - 1 )
                + margins.getFooterBottom() : 0 ) );
    for ( int colIndex = 0; colIndex < columns.length; colIndex++ )
