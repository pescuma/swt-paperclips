...
public interface PrintIterator {
   ...
   * <p>
   * If all of the remaining contents of the Print will fit in the given space,
   * the returned PrintPiece will include all remaining contents, and subsequent
   * calls to {@link PrintIterator#hasNext() } will return <code>false</code>.
   * <p>
   * If some, but not all of the remaining contents will fit in the given space,
   * the returned PrintPiece will contain as much of the contents as possible,
   * and subsequent calls to {@link PrintIterator#hasNext() } will return
   * <code>true</code>.
   * <p>
   ...
   *          iteration.
   * @return a PrintPiece that paints the next part of the Print, or null if the
   *         print area is too small. The size of the returned PrintPiece must
   *         NOT exceed the width and height indicated.
   */
   ...
   * This method allows a containing iterator to "back up" the current state of
   * its child iterators before invoking <code>next(int, int)</code> on them.
   * The containing iterator can then safely attempt iterating its child(ren) in
   * a variety of ways before selecting which way is the most appropriate.
